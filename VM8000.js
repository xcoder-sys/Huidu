...
文件名称: VM8000 Switcher (Simplified).js
功能: 音频处理器信号切换控制模块
信号：
  32个数字量输入：Pos1-Pos16 (输入信号), Pos17-Pos32 (输出通道)
  1个串行量输出：out$ (Pos1)

描述：
  模块按“选择信号输入 -> 选择输出通道”的顺序发送信号切换指令。
  指令格式为 "[x1]V[x2],[x3],[x4]."
作者: XuDaShuai
...
function call(MPV) {
	...
	if (lastSelectedInput !== null && outputChannelSelected !== null) {
		...
		command = lastSelectedInput + "V" + outputChannelSelected + ".";
